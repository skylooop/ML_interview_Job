...
flags.DEFINE_string(
    "speech_dir_valid",
    default="/home/m_bobrin/goznak/Task2Noise_Speech/Goznak_ML_Tasks/val/val/val/",
    help="Path to validation mel specs.",
)
...
def run_train(custom_model, ds_speech_loader_train, ds_speech_loader_val):
    ...
    for epoch in range(num_epochs):
        for x, y in tqdm(ds_speech_loader_train, leave=True, position=0):
            ...
            acc_train_metric = accuracy_score(
                y.detach().cpu().numpy(), acc_train.detach().cpu().numpy().ravel()
            )

            print(
                f"Epoch: {epoch}, Loss on current batch: {loss.item():.2f}, Accuracy Train batch: {acc_train_metric:.2f}",
                end="\r",
            )
            with torch.no_grad():
            ...
            if plotting % 50 == 0:
                acc_valid = accuracy_score(
                    ...
                    val_predict.detach().cpu().numpy().ravel(),
                )
                ...
                plt.grid()
                plt.savefig(
                    f"Task2Noise_Speech/assets/Step_{plotting}_epoch_{epoch}.jpg"
                )
